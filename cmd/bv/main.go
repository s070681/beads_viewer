...
import (
	...
	"github.com/Dicklesworthstone/beads_viewer/pkg/agent"
...
func main() {
	...
	// Agent Swarm Protocol flags
	robotAgents := flag.Bool("robot-agents", false, "List all registered agents as JSON")
	robotAgentRegister := flag.Bool("robot-agent-register", false, "Register this agent (reads from BV_AGENT_* env vars)")
	robotAgentHeartbeat := flag.Bool("robot-agent-heartbeat", false, "Update agent heartbeat timestamp")
	agentName := flag.String("agent-name", "", "Agent name for registration/heartbeat (overrides BV_AGENT_NAME)")
	agentModel := flag.String("agent-model", "", "Agent model for registration (overrides BV_AGENT_MODEL)")
	agentProgram := flag.String("agent-program", "", "Agent program for registration (overrides BV_AGENT_PROGRAM)")
	...
	if *robotHelp {
		...
		fmt.Println("")
		fmt.Println("  Agent Swarm Protocol (Multi-Agent Coordination)")
		fmt.Println("  ------------------------------------------------")
		fmt.Println("  --robot-agents")
		fmt.Println("      List all registered agents as JSON.")
		fmt.Println("      Output: {agents, total_active, total_claims, file_conflicts}")
		fmt.Println("      Use before starting work to see who else is active.")
		fmt.Println("")
		fmt.Println("  --robot-agent-register")
		fmt.Println("      Register this agent in the project.")
		fmt.Println("      Required: --agent-name or BV_AGENT_NAME env var")
		fmt.Println("      Required: --agent-program or BV_AGENT_PROGRAM env var")
		fmt.Println("      Optional: --agent-model or BV_AGENT_MODEL env var")
		fmt.Println("      Handles name collisions by assigning unique suffixes.")
		fmt.Println("      Example: bv --robot-agent-register --agent-name=claude-opus-1 --agent-program=claude-code")
		fmt.Println("")
		fmt.Println("  --robot-agent-heartbeat")
		fmt.Println("      Update agent's LastSeen timestamp (call every 5 minutes).")
		fmt.Println("      Required: --agent-name or BV_AGENT_NAME env var")
		fmt.Println("      Returns current claims and any file conflicts.")
		fmt.Println("      Agents inactive for 30+ minutes are marked inactive.")
		fmt.Println("      Example: bv --robot-agent-heartbeat --agent-name=claude-opus-1")
		fmt.Println("")
		fmt.Println("  Environment Variables for Agent Commands:")
		fmt.Println("      BV_AGENT_NAME    - Agent identifier (e.g., 'claude-opus-1')")
		fmt.Println("      BV_AGENT_MODEL   - Model name (e.g., 'claude-opus-4')")
		fmt.Println("      BV_AGENT_PROGRAM - Client program (e.g., 'claude-code', 'cursor')")
		fmt.Println("")
	...
	// Handle Agent Swarm Protocol commands (before loading issues)
	if *robotAgents || *robotAgentRegister || *robotAgentHeartbeat {
		handleAgentCommands(*robotAgents, *robotAgentRegister, *robotAgentHeartbeat,
			*agentName, *agentModel, *agentProgram)
		os.Exit(0)
	}
...
// ============================================================================
// Agent Swarm Protocol Handlers (bv-139)
// ============================================================================

// handleAgentCommands handles all agent-related robot commands
func handleAgentCommands(listAgents, registerAgent, heartbeat bool, name, model, program string) {
	...
	if listAgents {
		handleRobotAgents(store)
		return
	}

	if registerAgent {
		handleRobotAgentRegister(store, name, model, program)
		return
	}

	if heartbeat {
		handleRobotAgentHeartbeat(store, name)
		return
	}
}

// handleRobotAgents outputs all registered agents as JSON
func handleRobotAgents(store *agent.Store) {
	// Cleanup expired claims/hints first
	...
	for _, summary := range summaries {
		if summary.Status == agent.AgentStatusActive {
			activeCount++
		}
	...
	output := struct {
		...
		FileConflicts []agent.FileConflict `json:"file_conflicts,omitempty"`
	}{
		GeneratedAt: time.Now().UTC().Format(time.RFC3339),
		Agents:      summaries,
	...
	output.Explanation.What = "Active agent registry"
	output.Explanation.WhenToUse = "Before claiming work, to see who else is active"
	output.Explanation.Action = "Coordinate with listed agents to avoid conflicts"

	encoder := json.NewEncoder(os.Stdout)
	encoder.SetIndent("", "  ")
	if err := encoder.Encode(output); err != nil {
		fmt.Fprintf(os.Stderr, "Error encoding agents: %v\n", err)
		os.Exit(1)
	}
}

// handleRobotAgentRegister registers a new agent or updates an existing one
func handleRobotAgentRegister(store *agent.Store, name, model, program string) {
	// Get values from flags or environment
	if name == "" {
		name = os.Getenv("BV_AGENT_NAME")
	}
	if model == "" {
		model = os.Getenv("BV_AGENT_MODEL")
	}
	if program == "" {
		program = os.Getenv("BV_AGENT_PROGRAM")
	}

	// Validate required fields
	if name == "" {
		fmt.Fprintln(os.Stderr, "Error: Agent name required")
		fmt.Fprintln(os.Stderr, "Set via --agent-name flag or BV_AGENT_NAME environment variable")
		os.Exit(1)
	}
	if program == "" {
		fmt.Fprintln(os.Stderr, "Error: Agent program required")
		fmt.Fprintln(os.Stderr, "Set via --agent-program flag or BV_AGENT_PROGRAM environment variable")
		os.Exit(1)
	}

	// Create or update agent
	agentReg := agent.NewAgentRegistration(name, model, program)

	// Check for name collision and assign unique suffix if needed
	existing := store.Get(name)
	if existing != nil {
		// If different program, need unique name
		if existing.Program != program || existing.Model != model {
			// Find unique name with suffix
			baseName := name
			suffix := 1
			for store.Get(name) != nil {
	...
	if err := store.Register(agentReg); err != nil {
	...
	if err := store.Save(); err != nil {
		fmt.Fprintf(os.Stderr, "Error saving agents: %v\n", err)
		os.Exit(1)
	}
	...
	output.Agent.Name = agentReg.Name
	output.Agent.Model = agentReg.Model
	output.Agent.Program = agentReg.Program
	output.Agent.LastSeen = agentReg.LastSeen.UTC().Format(time.RFC3339)
	output.Explanation.What = "Agent registered successfully"
	output.Explanation.NextStep = fmt.Sprintf("Use --robot-agent-heartbeat --agent-name=%s every 5 minutes to stay active", agentReg.Name)
...
// handleRobotAgentHeartbeat updates an agent's LastSeen timestamp
func handleRobotAgentHeartbeat(store *agent.Store, name string) {
	// Get name from flag or environment
	if name == "" {
		name = os.Getenv("BV_AGENT_NAME")
	}
	...
	// Update heartbeat
	if err := store.Heartbeat(name); err != nil {
		fmt.Fprintf(os.Stderr, "Error updating heartbeat: %v\n", err)
		fmt.Fprintln(os.Stderr, "Agent may need to be registered first with --robot-agent-register")
		os.Exit(1)
	}

	// Cleanup expired claims/hints
	store.CleanupExpired()
	...
	agentInfo := store.Get(name)
	if agentInfo == nil {
		fmt.Fprintf(os.Stderr, "Error: Agent %s not found after heartbeat\n", name)
		os.Exit(1)
	}

	// Check for conflicts
	conflicts := store.GetFileConflicts()
	var myConflicts []agent.FileConflict
	for _, c := range conflicts {
		for _, a := range c.Agents {
			if a == name {
				myConflicts = append(myConflicts, c)
				break
			}
		}
	}
	...
	output := struct {
		...
		Conflicts []agent.FileConflict `json:"conflicts,omitempty"`
	}{
		GeneratedAt: time.Now().UTC().Format(time.RFC3339),
		Status:      "heartbeat_updated",
	}
	output.Agent.Name = agentInfo.Name
	output.Agent.LastSeen = agentInfo.LastSeen.UTC().Format(time.RFC3339)
	output.Agent.Status = string(agentInfo.Status)

	// Get active claim IDs
	activeClaims := agentInfo.ActiveClaims(time.Now())
	claimIDs := make([]string, len(activeClaims))
	for i, c := range activeClaims {
		claimIDs[i] = c.BeadID
	}
	output.Agent.ActiveClaims = claimIDs
	output.Conflicts = myConflicts
